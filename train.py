...
def run(args: Args):
    assert (
        args.from_checkpoint is not None if args.eval_mode else True
    ), "Must provide a checkpoint to evaluate from."
    assert (
        args.from_checkpoint.exists() if args.from_checkpoint is not None else True
    ), f"Checkpoint not found: {args.from_checkpoint}."
    assert (
        args.from_checkpoint.is_file() if args.from_checkpoint is not None else True
    ), f"Checkpoint must be a file but is not: {args.from_checkpoint}."
    ...
    if args.logdir is not None:
        ...
        logdir = (
            Path("./logdir/")
            / f'{"eval" if args.eval_mode else "training"}-{date}-{task_name}'
        )
    ...
    shutil.copy(
        task_path, logdir / task_path.name
    )  # Copy task file to logdir for reference

    logging.info(f"Args: {args}")

    logging.info("Creating DreamerV3 config")
    dreamer_config, step, logger = get_dreamer_config(
        logdir, args.dreamer_args, args.from_checkpoint
    )
...
def get_dreamer_config(
    logdir: Path, dreamer_args: str = "", from_checkpoint: Optional[Path] = None
):
...
def get_aai_env(
    task_path: Union[Path, str], env_path: Union[Path, str], dreamer_config
):
